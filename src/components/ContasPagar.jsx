...
const ContasPagar = ({ merceariaId, BACKEND_BASE_URL }) => {
...
    const [novaConta, setNovaConta] = useState({ descricao: '', valor: '', data_vencimento: '' });
...
            const response = await fetch(`${BACKEND_BASE_URL}/api/financeiro/pagar/${merceariaId}?status=${status}`);
            if (!response.ok) {
...
    // Roda a busca quando o componente/filtro muda
    useEffect(() => {
        fetchContas();
...
            
            // Rota de POST de CONTAS A PAGAR
...
            }
            
            setNovaConta({ descricao: '', valor: '', data_vencimento: '' }); 
            setShowForm(false); 
...
        
        try {
            // Rota de PUT para marcar como paga
            const response = await fetch(`${BACKEND_BASE_URL}/api/financeiro/pagar/${contaId}/pagar`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
...
                </div>
                
                <button className="btn-nova-conta" onClick={() => setShowForm(!showForm)}>
...
                        <input type="text" placeholder="Descrição" name="descricao" value={novaConta.descricao} required onChange={(e) => setNovaConta({...novaConta, descricao: e.target.value})} />
                        <input type="number" placeholder="Valor (R$)" name="valor" value={novaConta.valor} required onChange={(e) => setNovaConta({...novaConta, valor: e.target.value})} step="0.01" />
                        <input type="date" name="data_vencimento" value={novaConta.data_vencimento} required onChange={(e) => setNovaConta({...novaConta, data_vencimento: e.target.value})} />
                        
                        <div className="modal-actions-financeiro">
                            <button type="button" onClick={() => setShowForm(false)} disabled={saving}>Cancelar</button>
                            <button type="submit" disabled={saving || !novaConta.descricao || !novaConta.valor || !novaConta.data_vencimento}>{saving ? 'Salvando...' : 'Salvar Conta'}</button>
                        </div>
                    </form>
                </div>
            )}
...
                    <tbody>
                        {contas.length === 0 ? (
                            <tr><td colSpan="5" style={{ textAlign: 'center', padding: '20px' }}>Nenhuma conta a pagar encontrada para este filtro.</td></tr>
                        ) : (
                            contas.map(conta => (
                                <tr key={conta.id} className={conta.status === 'atrasada' || (conta.status === 'pendente' && new Date(conta.data_vencimento) < new Date()) ? 'conta-atrasada' : ''}>
                                    <td>{conta.descricao}</td>
                                    <td>{formatDate(conta.data_vencimento)}</td>
                                    <td>R$ {parseFloat(conta.valor).toFixed(2)}</td>
                                    <td className={`status-${conta.status.toLowerCase()}`}>{conta.status.toUpperCase()}</td>
                                    <td>
                                        {conta.status !== 'paga' && (
                                            <button onClick={() => handleMarcarPaga(conta.id)} disabled={saving} className="btn-pagar">
                                                {saving ? '...' : 'Marcar Pago'}
                                            </button>
                                        )}
                                    </td>
